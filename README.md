...
`Svga_SetPalette` for colours, `Svga_Close` to leave graphics, and the
`Svga_*`/`Graphics_*`/`Fonts_*` primitives for everything on screen. Every demo
also ships real image assets exercising the format loaders - BMP/PCX/GIF title
screens, ICO unit portraits, TIF HUD emblems, GIF monster art, CUR/ANI cursors -
generated by the per-demo scripts on top of `scripts/pbassets.py`.
